console.log("script");browser=function($){'use strict';var data=[{str:navigator.userAgent,sub:'Chrome',ver:'Chrome',name:'chrome'},{str:navigator.vendor,sub:'Apple',ver:'Version',name:'safari'},{prop:window.opera,ver:'Opera',name:'opera'},{str:navigator.userAgent,sub:'Firefox',ver:'Firefox',name:'firefox'},{str:navigator.userAgent,sub:'MSIE',ver:'MSIE',name:'ie'}];var v=function(s,n){var i=s.indexOf(data[n].ver);return(i!==-1)?parseFloat(s.substring(i+data[n].ver.length+1)):0;};var html=$('html');var result={name:'unknown',version:0};for(var n=0;n<data.length;n++){result[data[n].name]=false;if((data[n].str&&(data[n].str.indexOf(data[n].sub)!==-1))||data[n].prop){result.name=data[n].name;result[result.name]=true;result.version=v(navigator.userAgent,n)||v(navigator.appVersion,n);html.addClass(result.name+' '+result.name+parseInt(result.version,10)+' desktop');}}
return result;}(jQuery);jQuery(function($){'use strict';var i,j,k,l,m;if(!browser.ie||browser.version!==9){return;}
...
for(k=0;k<n.length;k++){s.addRule(n[k].s,n[k].v);}}}});jQuery(function($){'use strict';if(!browser.ie||browser.version>8)return;function split(str){str=str.replace(/"/g,'').replace(/%20/g,'');return str.split(/\s*,\s*/);}
$('.slider .slide-item').each(function(){var bgs=split($(this).css('background-image'));if(bgs.length>1){$(this).css("background-image",bgs[bgs.length-1]);}});});jQuery(function($){"use strict";if(!browser.ie||browser.version>8)return;$('.shapes').css('z-index',1);if(!browser.ie||browser.version>7)return;var textblockTexts=$('.textblock > div');textblockTexts.each(function(){var tbText=$(this);var valign=tbText.css('vertical-align')?tbText.css('vertical-align'):'top';if(valign==='middle'){var wrapper=tbText.wrap('<div/>').parent();tbText.css({'position':'relative','top':'-50%','height':'auto'});wrapper.css({'position':'absolute','top':'50%'});}else if(valign==='bottom'){tbText.css({'position':'absolute','height':'auto','bottom':0});}});});jQuery(function($){"use strict";var video=["youtube"];$("iframe[src]").each(function(){var iframe=$(this),src=iframe.attr("src"),isVideo=false,i;for(i=0;i<video.length;i++){if(src.toLowerCase().indexOf(video[i].toLowerCase())!==-1){isVideo=true;break;}}
if(!isVideo){return;}
if(src.lastIndexOf("?")!==-1){src+="&amp;wmode=transparent";}else{src+="?wmode=transparent";}
iframe.attr("src",src);});});jQuery(function($){"use strict";$(window).bind("resize",function(){navigatorResizeHandler($("html").hasClass("responsive"));});});var navigatorResizeHandler=(function($){"use strict";return function(responsiveDesign){if(responsiveDesign)return;$(".slider").each(function(){var slider=$(this);var sliderWidth=slider.width();var nav=slider.siblings(".slidenavigator");if(nav.length){var left=nav.attr("data-left");var margin=sliderWidth-sliderWidth*parseFloat(left)/100-nav.outerWidth(false);if(margin<0){nav.css("margin-left",margin);}}});};})(jQuery);jQuery(function($){"use strict";$(window).bind("resize",function(){"use strict";if(typeof responsiveDesign!=="undefined"&&responsiveDesign.isResponsive)
return;var sheetLeft=$(".sheet").offset().left;$("header.header #flash-area").each(function(){var object=$(this);object.css("left",sheetLeft+"px");});});});jQuery(function($){"use strict";$('nav.nav').addClass("desktop-nav");$('#horisontal_top_menu_mobile').click(function(){$('#navigation_top').toggleClass("mobile-nav");});});jQuery(function($){"use strict";if(!browser.ie||browser.version>7){return;}
...
showArrows();showLoader(true);bindMouse($(".lightbox-wrapper").add(target));target.load(function(){showLoader(false);active.removeClass("active");target.addClass("active");});target.error(function(){showLoader(false);active.removeClass("active");target.addClass("active");target.attr("src",$(images[current]).attr("src"));});}
...
e.preventDefault();});}
function getFullImgSrc(src){var fileName=src.substring(0,src.lastIndexOf('.'));var ext=src.substring(src.lastIndexOf('.'));return fileName+"-large"+ext;}});})(jQuery);jQuery(function(){'use strict';new ThemeLightbox().init();});(function($){'use strict';$.support.transition=(function(){var thisBody=document.body||document.documentElement,thisStyle=thisBody.style,support=thisStyle.transition!==undefined||thisStyle.WebkitTransition!==undefined||thisStyle.MozTransition!==undefined||thisStyle.MsTransition!==undefined||thisStyle.OTransition!==undefined;return support&&{event:(function(){var e="transitionend";if(browser.opera){var version=browser.version;e=version>=12?(version<12.50?"otransitionend":"transitionend"):"oTransitionEnd";}else if(browser.chrome||browser.safari){e="webkitTransitionEnd";}
return e;})(),prefix:(function(){return({opera:"-o-",firefox:"-moz-",chrome:"-webkit-",safari:"-webkit-",ie:"-ms-"}[browser.name]);})()};})();window.BackgroundHelper=function(){var slides=[];var direction="next";var motion="horizontal";var width=0;var height=0;var multiplier=1;var transitionDuration="";this.init=function(motionType,dir,duration){direction=dir;motion=motionType;slides=[];width=0;height=0;multiplier=1;transitionDuration=duration;};this.processSlide=function(element,modify){this.updateSize(element,null);var pos=[];var bgPosition=element.css("background-position");var positions=bgPosition.split(",");$.each(positions,function(i){var position=$.trim(this);var point=position.split(" ");if(point.length>1){var x=parseInt(point[0],10);var y=parseInt(point[1],10);pos.push({x:x,y:y});}});slides.push({"images":element.css("background-image"),"sizes":element.css("background-size"),"positions":pos});if(modify)
...
if(typeof arg==="string"&&data[arg]){data[arg]();}else if(data.settings.auto&&element.is(":visible")){data.start();}});};})(jQuery);jQuery(window).bind("resize",(function($){"use strict";return function(){if(typeof responsiveDesign!=="undefined"&&responsiveDesign.isResponsive){$("header.header .shapes").children().css("left","");return;}
var sheetWidth=$(".sheet").width();var sheetLeft=$(".sheet").offset().left;$("header.header .shapes>*, header.header>.textblock, header.header>.headline, header.header>.slogan, header.header>.positioncontrol").each(function(){var object=$(this);var objectLeft=sheetWidth*parseFloat(object.attr("data-left")||"0")/100+sheetLeft;object.css("left",objectLeft+"px");});};})(jQuery));jQuery(function($){"use strict";$(window).trigger("resize");});jQuery(function($){"use strict";if(!browser.ie||browser.version>8)
...
header.css('background-image',"url('images/header.png')".replace(/(url\(['"]?)/i,"$1"+path));header.css('background-position',"center top");});})(jQuery);(function($){'use strict';$(function(){artButtonSetup("btn-primary");artButtonSetup("button");artButtonSetup("readon");artButtonSetup("readmore");});$(function($){var links=$('a.rss-tag-icon[href="#"], a.rss-tag-icon[href=""]'),href=$('head link[type="application/rss+xml"]').attr('href');if(href){links.attr('href',href);}else{links.hide();}});$(function($){$('.messages').each(function(){$(this).css('display',$(this).find('dl, .alert').length?'':'none');});});$(function($){$('.search-button').each(function(i,val){var element=$(val),background=element.css('background-image');if(-1!==background.indexOf('url')){element.val('');}
if(element.hasClass('button')){element.removeClass('button');}});});$(function($){if(!browser.ie||browser.version>8){return;}
var scriptSrc=jQuery('script[src*="script.js"]').last().attr('src'),pathToImages=scriptSrc.substr(0,scriptSrc.lastIndexOf('/')+1)+'data/images/';$('.slider .slide-item').each(function(){var bg=$(this).css('background-image');if(bg){bg=bg.replace(/url\(["'](.+)["']\)/,function(match,path){var imageName=path.substr(path.lastIndexOf('/')+1);return'url(\''+pathToImages+imageName+'\')';});$(this).css('background-image',bg);}});});}(jQuery));jQuery(document).ready(function(){jQuery('input[name=phone],input#phone').mask('+7 (999) 999-99-99');jQuery("input, textarea").focus(function(){if(jQuery(this).attr("placeholder")==jQuery(this).attr("title"))
jQuery(this).attr("placeholder","")});jQuery("input, textarea").blur(function(){if(jQuery(this).attr("placeholder")=="")
jQuery(this).attr("placeholder",jQuery(this).attr("title"))});jQuery(".clients-slider").jcarousel({wrap:'circular',}).jcarouselAutoscroll({interval:5000,autostart:true});jQuery('.cl-arr-l').on('jcarouselcontrol:active',function(){jQuery(this).removeClass('inactive');}).on('jcarouselcontrol:inactive',function(){jQuery(this).addClass('inactive');}).jcarouselControl({target:'-=1'});jQuery('.cl-arr-r').on('jcarouselcontrol:active',function(){jQuery(this).removeClass('inactive');}).on('jcarouselcontrol:inactive',function(){jQuery(this).addClass('inactive');}).jcarouselControl({target:'+=1'});jQuery('.show_comment_all').click(function(e){e.preventDefault();if(jQuery(this).hasClass('active')){jQuery(this).removeClass('active');jQuery(this).text("РїРѕРєР°Р·Р°С‚СЊ РїСЂРѕР»РЅРѕСЃС‚СЊСЋ");jQuery(this).parent().find('.all_comment').addClass('unvisible');jQuery(this).parent().find('.intro_comment').removeClass('unvisible');}
else{jQuery(this).addClass('active');jQuery(this).text("СЃРєСЂС‹С‚СЊ");jQuery(this).parent().find('.all_comment').removeClass('unvisible');jQuery(this).parent().find('.intro_comment').addClass('unvisible');}});let data=new Date();jQuery('.year').text(data.getFullYear())
jQuery('.spoiler_item .spoiler_title').click(function(){let parent=jQuery(this).parent('.spoiler_item');if(parent.hasClass('spoiler_show')){parent.removeClass('spoiler_show')}else{jQuery('.spoiler_item.spoiler_show').removeClass('spoiler_show')
parent.addClass('spoiler_show')
setTimeout(()=>{jQuery('html, body').animate({scrollTop:parent.offset().top-(parent.children('.spoiler_title').height())},300);},300)}})
Vue.config.devtools=true;Vue.component('card',{template:`
    <a class="card-wrap"
    :href="cardLink"
    ...
    </a>`,mounted(){this.width=this.$refs.card.offsetWidth;this.height=this.$refs.card.offsetHeight;this.p_height=this.$refs.box.lastElementChild.offsetHeight;this.box=this.$refs.box;},props:['dataImage','dataHref'],data:()=>({width:0,height:0,mouseX:0,mouseY:0,mouseLeaveDelay:null}),computed:{mousePX(){console.log('mousePX',this.mouseX,this.width)
return this.mouseX/this.width;},mousePY(){console.log('mousePY',this.mouseY,this.height)
return this.mouseY/this.height;},cardStyle(){const rX=this.mousePX*30;const rY=this.mousePY*-30;return{transform:`rotateY(${rX}deg) rotateX(${rY}deg)`};},cardBgTransform(){const tX=this.mousePX*-40;const tY=this.mousePY*-40;return{transform:`translateX(${tX}px) translateY(${tY}px)`};},cardBgImage(){return{backgroundImage:`url(${this.dataImage})`};},cardLink(){return(this.dataHref)},heightInfo(){this.height
return{transform:` translateY(${this.p_height}px)`};}},methods:{handleMouseMove(e){this.mouseX=e.pageX-this.$refs.card.offsetLeft-this.width/2;this.mouseY=e.pageY-this.$refs.card.offsetTop-this.height/2;},handleMouseEnter(){this.box.style.transform=`translateY(0px)`
clearTimeout(this.mouseLeaveDelay);},handleMouseLeave(){this.box.style.transform='translateY('+this.p_height+'px)'
this.mouseLeaveDelay=setTimeout(()=>{this.mouseX=0;this.mouseY=0;},1000);}}});const app=new Vue({el:'.card_init'});});jQuery(function($){var getElementsByClassName=function(className,tag,elm){if(document.getElementsByClassName){getElementsByClassName=function(className,tag,elm){elm=elm||document;var elements=elm.getElementsByClassName(className),nodeName=(tag)?new RegExp("\\b"+tag+"\\b","i"):null,returnElements=[],current;for(var i=0,il=elements.length;i<il;i+=1){current=elements[i];if(!nodeName||nodeName.test(current.nodeName)){returnElements.push(current);}}
...
return returnElements;};}
return getElementsByClassName(className,tag,elm);};function reload_table(){var headertext=[],headers=document.querySelectorAll(".table-mobile tr:first-child td"),tablerows=document.querySelectorAll(".table-mobile th"),tablebody=document.querySelectorAll(".table-mobile tbody");console.log(tablebody[0].rows);for(var i=0;i<headers.length;i++){var current=headers[i];headertext.push(current.textContent.replace(/\r?\n|\r/," "));}
for(var h=0;h<tablebody.length;h++){for(var i=0,row;row=tablebody[h].rows[i];i++){for(var j=0,col;col=row.cells[j];j++){col.setAttribute("data-th",headertext[j]);}}}}
reload_table();});